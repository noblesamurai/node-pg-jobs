...
module.exports = function(options) {
  ...
  Jobs.maybeProcessNow = function(jobId, processFunction, done) {
    return Jobs.processNow(jobId, processFunction, done, true);
  };

  Jobs.processNow = function(jobId, processFunction, done, nonblocking) {
    done = done || function() {};
    pg.connect(options.db, connected);
    ...
    function connected(err, db, releaseClient) {
      ...
      if (nonblocking) {
        jobsModel.maybeObtainLock(db, jobId, gotLock);
      } else {
        jobsModel.obtainLock(db, jobId, gotLock);
      }

      function gotLock(err, lock) {
        if (err) {
          debug('failed to obtained lock for', jobId, err);
          return done(err);
        } else if (nonblocking && !lock) {
          debug('job busy');
          err = new Error('Job busy');
          err.type = 'job_busy';
          return done(err);
        }
    ...
    function processJob(jobSnap) {
      ...
      serviceJob(tx, jobSnap, processFunction, function(err) {
        ...
        function txDone() {
          ...
          jobsModel.unlock(db, jobSnap.job_id);
          callback(err);
        }
      });
    }
