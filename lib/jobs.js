...
if (process.env.NODE_ENV == 'test') {
  exports.setJobs = function(db, newJobs, callback) {
    jobsModel.setJobs(db, newJobs, callback);
  };
  exports.getJobs = function(db, callback) {
    return jobsModel.getJobs(db, callback);
  };
  exports.getScheduledJobs = function(db, callback) {
    jobsModel.scheduledJobs(db, callback);
  };
  exports.removeAllJobs = function(db, callback) {
    db.query('delete from jobs;', callback);
  };
}

exports.eventEmitter = new events.EventEmitter();

exports.create = function(db, jobData, processIn, done) {
        ...
        function serviceJobHandler(err) {
          if (err) {
            ...
            db.query('commit;', function(err) {
              if (err) return cb(err);
              exports.eventEmitter.emit('processCommitted');
              cb();
            });
          }
        }
...
exports.processNow = function(db, jobId, callback, done) {
  db.query('begin', getLock);

  function getLock(err) {
    if (err) return done(err);
    jobsModel.obtainLock(db, jobId, gotResult);
    exports.eventEmitter.emit('lockSought');
  }

  function gotResult(err, jobSnap) {
    if (err) return done(err);
    exports.eventEmitter.emit('lockObtained');
    console.log('jobSnap:');
    console.log(jobSnap);
  ...
  function commitTransaction() {
    ...
    db.query('commit', function(err) {
      if (err) return done(err);
      exports.eventEmitter.emit('processNowCommitted');
      done();
    });
  }
};
